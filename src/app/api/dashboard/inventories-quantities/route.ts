...
export async function GET() {
  try {
    ...
    const allInventoryNames = Array.from(new Set(items.filter(item => item.inventory).map(item => item.inventory!.name)));
    ...
    const chartData = Object.keys(monthlyData)
      .sort((a, b) => { // Sort by date chronologically
    ...
    return NextResponse.json({ error: "Failed to fetch item quantity data" }, { status: 500 });
  }
}