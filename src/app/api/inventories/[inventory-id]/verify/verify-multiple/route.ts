  ...
  try {
    ...
    if (itemsToVerify.length === 0) {
      return NextResponse.json(
        {
          error:
            "No matching items found or unauthorized to verify the specified items.",
        },
        { status: 404 }
      );
    }

    const actualIdsToVerify = itemsToVerify.map((item) => item.id);
    ...
    return NextResponse.json(
      {
        message: `${itemsToVerify.length} items verified successfully.`,
      },
      { status: 200 }
    );
  } catch (error) {
